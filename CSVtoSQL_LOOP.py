import sqlite3          # to set up: pip install db-sqlite3.... # from sqlite3 import Error .. I'm not doing error handling this is a one off 
import csv              # to set up: pip install python-csv

conn= sqlite3.connect('example.db')    # This is the Database, the database then has a table(s)

f = open('FileCount.txt', 'r')             # By running iterate_to_text.py FIRST, then we know all the filenames and how many there are.
nFILECOUNT = f.readline(); print (nFILECOUNT," files to be processed"); f.close()
