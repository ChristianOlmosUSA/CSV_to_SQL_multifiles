...
''')                # Create an SQL table
     #   ***rel path: smallTestFolder\E1AF1.C1800.csv

f = open( "FileDirectory.txt", "r" )            # 
lines = []
for line in f:
    lines.append(line)
    line = line.strip('\n')
    fileloc = str(line)
    print(fileloc)
                                                # Note: All data is imported as a string "str(row[])", so to do maths you need to change it into a number like: float(OpenBidPrice)-float(CloseBigPrice)
    with open(fileloc) as csv_file:                # So the SQL is inside the '''  ''' part, whereas the pythonic can be a list like here without a , each time
